...
export class ServerService {
  ...
  filter$ = (status: Status, response: CustomResponse) => <Observable<CustomResponse>> new Observable<CustomResponse>(
    subscriber => {
      ...
      subscriber.next(
        status === Status.ALL ? { ...response, message: `Servers filtered by ${status} status`} : { ...response, message: (response.data.servers as Server[]).filter(server => server.status === status).length  > 0 ? `Servers filtered by ${status === Status.SERVER_UP ? 'SERVER UP' : 'SERVER DOWN'} status` : `No servers of ${status} found`, data: { servers: (response.data.servers as Server[]).filter(server => server.status === status) }}
      );
      subscriber.complete();
    }
  )
